...
contract MainContract is ApproverRole, ReentrancyGuard {
    ...
    function setPuan(uint256 _puan, address payable _freelancerAddress) external {
        ...
        returns (bool, address)
    {
        ...
        if (block.timestamp > deployedWork.deadLine() && deployedWork.deadLine() != 0) {
...
contract WorkContract is ApproverRole, ReentrancyGuard {
        ...
        mustActive
    {
        ...
        deployedFromContract.setFreelancerWorkAddress(address(this), payable(_freelancerAddress));
        if (data.ArgonShield) {
            require(
            ...
            deployedFromContract.setApproverWorkAddress(address(this), _approveraddress);
        }
